...
public func assertNotNil(
    ...
    if variable == nil {
        failTest(
            "Expected non-nil value but got [\(variable.asString)]]",
            filename: filename,
            lineNumber: lineNumber
        )
    }
...
public func assertNil(
    ...
    if let variable = variable {
        failTest(
            "Expected nil value, but [\(variable)]",
            filename: filename,
            lineNumber: lineNumber
        )
    }
...
public func assertEquals(
    ...
    assertEquals(
        Double(first),
        Double(second),
        marginOfError: Double(marginOfError),
        filename: filename,
        lineNumber: lineNumber
    )
}
...
public func assertEquals(
    ...
    assertEquals(
        Decimal(first),
        Decimal(second),
        marginOfError: Decimal(marginOfError),
        filename: filename,
        lineNumber: lineNumber
    )
}
...
public func assertEquals(
    _ first: Date,
    _ second: Date,
    marginOfError: TimeInterval,
    filename: StaticString = #file,
    lineNumber: UInt = #line
) {
    assertEquals(
        Double(first.timeIntervalSince1970),
        Double(second.timeIntervalSince1970),
...
public func assertNotEquals<T: Equatable>(
    ...
    guard let first, let second else {
        if first == second {
            failTest(
                "Expected different values, but [\(first.asString) & \(second.asString)] are the same.",
                filename: filename,
                lineNumber: lineNumber
            )
        }
...
public func assertNotEmpty(
    ...
    if string == nil || string!.isEmpty {
        failTest(
            "String is empty: [\(string.asString)]",
            filename: filename,
            lineNumber: lineNumber
        )
    }
...
public func assertNotEmpty(
    _ collection: AnyCollection<Any>,
    filename: StaticString = #file,
    lineNumber: UInt = #line
) {
...
public func assertEmpty(
    ...
    if !collection.isEmpty {
        failTest(
            "Collection is not empty. Has \(collection.count) elements",
            filename: filename,
            lineNumber: lineNumber
        )
    }
...
public func assertEmpty(
    ...
    if !array.isEmpty {
        failTest(
            "Array is not empty. Has \(array.count) elements",
            filename: filename,
            lineNumber: lineNumber
        )
    }
